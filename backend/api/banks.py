"""Bank connection endpoints (GoCardless, Phase 1).

Flow: list institutions -> create a requisition (returns a consent link) -> user authenticates
at their bank and is redirected to /banks/callback (or you POST .../finalize) -> accounts are
imported -> sync pulls balances + transactions. Connections can be listed and purged.
"""
...
from backend.api.deps import GoCardlessClientDep, SessionDep
...
def list_institutions(client: GoCardlessClientDep, country: str | None = None) -> list[dict]:
...
def create_requisition(
    payload: RequisitionCreate, session: SessionDep, client: GoCardlessClientDep
) -> RequisitionCreateOut:
    settings = get_settings()
...
def finalize_requisition(
    requisition_id: str, session: SessionDep, client: GoCardlessClientDep
) -> FinalizeOut:
    connection = repository.get_connection_by_requisition(session, requisition_id)
...
def consent_callback(
    session: SessionDep, client: GoCardlessClientDep, ref: str | None = None
) -> FinalizeOut:
    ...
    connection = repository.get_connection_by_reference(session, ref)
    if connection is None:
...
def list_connections(session: SessionDep) -> list[ConnectionOut]:
    return [
        ConnectionOut.model_validate(c)
        for c in repository.list_connections(session, connector=BankConnector.name)
    ]


@router.post("/connections/{connection_id}/sync", response_model=SyncResultOut)
def sync(
    connection_id: uuid.UUID, session: SessionDep, client: GoCardlessClientDep
) -> SyncResultOut:
    connection = repository.get_connection(session, connection_id)
    if connection is None:
        raise HTTPException(status_code=404, detail="unknown connection")
    result = sync_connection(session, connection, BankConnector(session, client))
...
def delete_connection(connection_id: uuid.UUID, session: SessionDep) -> Response:
    """Purge a connection and all of its accounts/balances/transactions (§8)."""
    if not repository.delete_connection(session, connection_id):
        raise HTTPException(status_code=404, detail="unknown connection")
    session.commit()
