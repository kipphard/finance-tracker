...
import type { Cadence, CashflowItemOut, CashflowSummaryOut } from "../api/types";
...
function CashflowForm({
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      await onSubmit({ name, amount, cadence });
      onClose();
    } catch (err) {
...
export function CashflowCard({ className }: { className?: string }) {
  ...
  const actions = (
    <>
      <button className="btn btn--sm" onClick={() => setModal({ direction: "inflow" })}>
        + Inflow
      </button>
      <button
        className="btn btn--sm btn--ghost"
        onClick={() => setModal({ direction: "outflow" })}
      >
        + Outflow
      </button>
    </>
  ...
  return (
    <Card title="Monthly cashflow" className={className} action={actions}>
      <Async state={summary}>
        {(s) => (
          <>
      ...
      <Async state={items}>
        {(list) => {
          ...
          if (active.length === 0)
            return (
              <div className="empty">
                No recurring items yet — add an inflow or outflow above.
              </div>
            );
          return (
              ...
              {active.map((i) => (
                <li key={i.id}>
                  <span
                    ...
                    onClick={() => setModal({ direction: i.direction, item: i })}
                    title="Edit"
                  >
                    <span className="li-main">{i.name}</span>{" "}
                    <span className="li-sub">· {titleCase(i.cadence)}</span>
                  </span>
                  <span style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    <span
                      className={"tnum " + (i.direction === "inflow" ? "pos" : "neg")}
                    >
                      {i.direction === "inflow" ? "+" : "−"}
