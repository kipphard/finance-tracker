...
def test_seed_and_category_crud(client):
    seeded = client.post("/categories/seed").json()
    assert len(seeded) == 15
    # Idempotent: a second seed creates nothing new.
    assert client.post("/categories/seed").json() == []

    created = client.post("/categories", json={"name": "Hobbies", "kind": "expense"})
    assert created.status_code == 201
    # Duplicate name rejected.
    assert client.post("/categories", json={"name": "Hobbies", "kind": "expense"}).status_code == 409
...
def test_rule_requires_existing_category(client):
    ...
    bad = client.post(
        "/rules", json={"match_pattern": "x", "category_id": str(uuid.uuid4())}
    )
...
def test_reclassify_and_remember_creates_rule(client):
    account_id = _seed_and_account(client)
    dining = next(c for c in client.get("/categories").json() if c["name"] == "Dining")["id"]

    txn_id = client.post(
        f"/accounts/{account_id}/transactions",
        json={"ts": "2026-03-01T00:00:00Z", "amount": "-30.00", "raw_payee": "Pizza Place"},
    ).json()["id"]
    assert client.get(f"/transactions/{txn_id}").json()["category_id"] is None

    patched = client.patch(
        f"/transactions/{txn_id}", json={"category_id": dining, "remember": True}
    )
