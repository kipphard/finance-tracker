...
def test_add_items_and_summary(client):
    ...
    summary = client.get("/api/cashflow/summary").json()
...
def test_list_filter_patch_delete(client):
    ...
    inflows = client.get("/api/cashflow", params={"direction": "inflow"}).json()
    assert len(inflows) == 1
    assert inflows[0]["name"] == "Salary"

    # Deactivate the salary -> drops out of the summary.
    client.patch(f"/api/cashflow/{inflow_id}", json={"active": False})
    assert client.get("/api/cashflow/summary").json()["monthly_inflow"] == "0.00"

    assert client.delete(f"/api/cashflow/{inflow_id}").status_code == 204
    assert client.get(f"/api/cashflow/{inflow_id}").status_code == 404
