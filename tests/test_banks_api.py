...
def test_full_bank_flow_via_api(bank_client):
    institutions = bank_client.get("/api/banks/institutions").json()
    assert institutions[0]["id"] == "BANK_DE"

    created = bank_client.post("/api/banks/requisitions", json={"institution_id": "BANK_DE"})
    ...
    finalized = bank_client.post("/api/banks/requisitions/req-1/finalize")
    assert finalized.status_code == 200
    assert len(finalized.json()["accounts"]) == 2

    synced = bank_client.post(f"/api/banks/connections/{connection_id}/sync").json()
    assert synced["new_transactions"] == 1
    assert synced["balances_recorded"] == 2

    net_worth = bank_client.get("/api/networth").json()
    assert Decimal(str(net_worth["total"])) == Decimal("1250.50")

    connections = bank_client.get("/api/banks/connections").json()
    assert len(connections) == 1
    assert connections[0]["status"] == "active"

    assert bank_client.delete(f"/api/banks/connections/{connection_id}").status_code == 204
    after = bank_client.get("/api/networth").json()
